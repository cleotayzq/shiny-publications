...
publications <- read.csv("list_current.csv")

coauthors <- publications$author %>%
    strsplit(., " and ") %>%
    unlist() %>%
    subset(., . != "Chachan, Yayaati") %>%
    table() %>%
    # unique() %>%
    sort(decreasing = T) %>%
    as.data.frame() %>%
    rowwise() %>%
    filter(Freq >= 4 | # including authors who've coauthored at least 4 papers
               # and a few more
               `.` == "Stevenson, David J." |
               `.` == "Clarke, Cathie" |
               `.` == "Zhang, Michael") %>% 
...
# pull keywords from bibtex file
        ...
        sidebarPanel(
            fluidRow(column(width = 12,
                    ...
                    sliderTextInput("authorship", "Filter by authorship order:",
                                    ...
                                    grid = TRUE)
                    # actionButton("show_all", "Show all papers") # moved to main panel
            )),
            fluidRow(column(width = 12,
            ...
            fluidRow(column(width = 12,
                checkboxGroupButtons("coauthor_filter", "Filter by co-author(s):",
                                     ...
                                     direction = "horizontal", size = "xs"),
                # checkboxGroupInput("coauthor_filter", "Filter by co-author(s):",
                #                      choices = coauthors, selected = coauthors),
...
server <- function(input, output, session) {
    ...
    filtered_data <- reactive({
        ...
        filtered <- publications %>%
            ...
            rowwise() %>%
            filter(any(input$keyword_filter %in% tolower(unlist(strsplit(keywords, ", "))))) %>%
            filter(any(input$coauthor_filter %in% unlist(strsplit(author, " and ")))) %>%
            ungroup()
    ...
    observeEvent(input$show_all, {
        ...
        # updateCheckboxGroupInput(session, "coauthor_filter", selected = coauthors)
    ...
    observeEvent(input$no_coauthors, {
        ...
        updateCheckboxGroupInput(session, "coauthor_filter", selected = "")
    })
